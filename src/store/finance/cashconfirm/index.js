...
const store = {
    ...
    state: {
        ...
        isbatchAuditLoading: false, // 批量转账loading
        isbatchDeleteLoading: false,// 批量废除loading
        isrefreshStatusLoading: false,// 刷新状态loading
        isrePayLoading: false,// 重新支付loading
        ismanualConfirmLoading: false,// 人工确认loading
        manualId: null, // 人工确认id,
    ...
    actions: {
        ...
        async getListData({ commit, state }) {
            ...
            const response = await api.getCashList(reqParams);
        ...
        async batchCash({ dispatch, commit }, payload) {
            ...
            const response = await api.batchCash(payload);
            if (auditResult === 0) {
        ...
        async refreshStatus({ dispatch, commit }, payload) {
            commit("overrideStateProps", { isrefreshStatusLoading: true });
            const response = await api.refreshStatus(payload);
            commit("overrideStateProps", { isrefreshStatusLoading: false });
            commit("overrideStateProps", { refreshStatusVisible: false });
            switch (response.code) {
                case DICT.SUCCESS:
                    Message.success(response.mesg);
                    break;
        ...
        async rePay({ dispatch, commit }, payload) {
            commit("overrideStateProps", { isrePayLoading: true });
            const response = await api.rePay(payload);
            commit("overrideStateProps", { isrePayLoading: false });
            commit("overrideStateProps", { rePayVisible: false });
            switch (response.code) {
                case DICT.SUCCESS:
                    Message.success(response.mesg);
                    break;
        ...
        async cashierConfirm({ dispatch, commit }, payload) {
            commit("overrideStateProps", { ismanualConfirmLoading: true });
            const response = await api.cashierConfirm(payload);
            commit("overrideStateProps", { ismanualConfirmLoading: false });
            commit("overrideStateProps", { manualId: null });
            switch (response.code) {
                case DICT.SUCCESS:
                    Message.success(response.mesg);
                    break;
